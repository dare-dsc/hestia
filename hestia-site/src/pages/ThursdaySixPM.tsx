...
export default function ThursdaySixPM() {
  ...
  return (
    <div ref={rootRef} className="theme-thursday relative">
      ...
      {/* Chrome */}
      <header className="fixed top-3 inset-x-3 md:top-5 md:inset-x-6 z-40 flex items-center justify-between rounded-2xl px-4 md:px-6 py-2.5 md:py-3 bg-white/15 backdrop-blur-xl backdrop-saturate-150 border border-white/30 shadow-[0_8px_32px_-12px_rgba(43,36,32,0.25)]">
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            className="hidden sm:inline-flex items-center gap-2 rounded-full border border-[var(--line)] bg-[var(--surface)]/85 text-[var(--ink)] px-4 py-2 text-sm font-semibold tracking-tight transition-transform active:scale-[0.97] active:translate-y-[1px]"
          >
      ...
      <section ref={heroRef} className="relative h-[100dvh] overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={img("hestia-kitchen-nairobi", 1920, 1200)}
            alt=""
            className="h-full w-full object-cover"
          />
          <div className="absolute inset-0 bg-[var(--ink)]/42" />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(43,36,32,0.06)_0%,rgba(43,36,32,0.34)_78%)]" />
          <div
            ref={heroRevealRef}
        ...
        <div
          ref={leftDoorRef}
          className="absolute inset-y-0 left-0 w-1/2 bg-[var(--clay)] flex flex-col items-end justify-center pr-4 md:pr-10 text-right"
        >
          <span className="font-mono text-xs md:text-sm uppercase tracking-[0.3em] text-[var(--surface)]/70 mb-2 md:mb-4">
            Nairobi · Thu
          </span>
          <h1 className="font-display text-[13vw] md:text-[7.5vw] leading-[0.95] text-[var(--surface)]">
            It&rsquo;s
            <br />
            6pm.
          </h1>
        ...
        <div
          ref={rightDoorRef}
          className="absolute inset-y-0 right-0 w-1/2 bg-[var(--ink)] flex flex-col items-start justify-center pl-4 md:pl-10"
        >
          <h1 className="font-display text-[13vw] md:text-[7.5vw] leading-[0.95] text-[var(--surface)]">
            You&rsquo;re
            <br />
          ...
          <span className="font-mono text-xs md:text-sm uppercase tracking-[0.3em] text-[var(--surface)]/70 mt-2 md:mt-4">
            Every time
          </span>
        </div>
        ...
        <div
          ref={heroChromeRef}
          className="absolute inset-x-0 bottom-28 flex flex-col items-center gap-3 text-[var(--surface)] md:bottom-10"
        >
          <p className="font-body text-sm md:text-base text-center max-w-xl px-6 text-[var(--surface)]/85">
            Hestia keeps this house running on schedule, with the checks,
            terms, and follow-up handled before you get home.
          </p>
          <div className="hidden md:flex items-center gap-2 rounded-full border border-white/15 bg-[var(--surface)]/10 px-3 py-2 backdrop-blur-md shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
            {OFFER_POINTS.map((point) => (
              <span
                key={point}
                className="rounded-full bg-[var(--surface)]/12 px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-[var(--surface)]/82"
              >
                {point}
              </span>
            ))}
          </div>
          <span className="flex items-center gap-2 text-xs uppercase tracking-[0.25em] text-[var(--surface)]/60">
            Scroll to walk in
      ...
      <main className="relative z-10 bg-[var(--bg)]">
        ...
        {/* SEVEN CHECKS */}
        <section className="checks-section relative overflow-hidden bg-[var(--plum-deep)] py-20 md:py-32">
          <div
            className="checks-spotlight pointer-events-none absolute inset-x-0 -top-1/4 h-[150%]"
            style={{
              background:
                "radial-gradient(ellipse 65% 55% at 50% 20%, var(--plum) 0%, transparent 70%)",
            }}
          />
          <div
          ...
          <div className="relative mx-auto max-w-7xl px-5 md:px-10">
            <div className="reveal max-w-2xl mb-14 md:mb-20">
              ...
              <h2 className="font-display text-4xl md:text-6xl leading-[1.05] mb-5 text-[var(--surface)]">
                Seven checks.
                <br />
                Stamped before anyone starts.
              </h2>
              <p className="text-base md:text-lg leading-relaxed text-[var(--surface)]/75">
            ...
            <div className="checks-list grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
              {TRUST_DIMENSIONS.map((dim, i) => (
                ...
                >
                  <div
                    className="absolute -top-4 right-5 md:right-6 flex h-[4.5rem] w-[4.5rem] items-center justify-center rounded-full border-2 border-dashed border-[var(--clay)] bg-[var(--plum-deep)] md:h-20 md:w-20"
                    style={{
                  ...
                  <p className="font-display text-xl md:text-2xl leading-snug text-[var(--surface)] mt-2 mb-2">
                    {dim.label}
                  </p>
                  <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--surface)]/55 mb-3">
                    {dim.question}
                  </p>
                  <p className="text-sm leading-relaxed text-[var(--surface)]/75">
                    {dim.detail}
                  </p>
                </div>
              ))}
              ...
              <div className="check-row relative rounded-sm border border-dashed border-[var(--clay)]/50 bg-white/[0.07] p-6 md:p-7 shadow-[0_24px_60px_-40px_rgba(0,0,0,0.55)]">
                ...
                <div className="space-y-3 text-sm">
                  <p>
                    <span className="block font-semibold text-[var(--surface)]">
                      Before start
                    </span>
                    <span className="text-[var(--surface)]/75">
      ...
      <div className="fixed bottom-3 inset-x-3 z-40 grid grid-cols-2 gap-2 rounded-2xl border border-white/40 bg-[var(--surface)]/90 p-2 shadow-[0_18px_50px_-24px_rgba(43,36,32,0.35)] backdrop-blur-xl md:hidden">
        <a
          href={`tel:${CONTACT.phone.replace(/\s/g, "")}`}
          className="inline-flex items-center justify-center gap-2 rounded-full border border-[var(--line)] bg-[var(--surface)] px-4 py-3 text-sm font-semibold text-[var(--ink)] transition-transform active:scale-[0.97]"
        >
          <Phone size={15} weight="fill" className="text-[var(--clay)]" />
          Call
        </a>
